...
char *str_normalise(const char *str, bool to_upper, bool to_lower);
...
void uniform_init(shaderblock_t* sb_t) {
    ...
    const char* shader_sources[] = {
        sb_t->vertexshader_code,
        sb_t->fragmentshader_code,
        sb_t->geometryshader_code
    };
...
void uniform_write(shaderblock_t* shader, const char* type, const char* name, const char* property, bool* transpose, const void* value, size_t num_elements) {
    ...
    for (size_t cc = 0; cc < shader->uniform_len; ++cc) {
        if (strcmp(name, shader->uniforms[cc].name) == 0) {
            location = shader->uniforms[cc].id;
            break;
        }
    }
...
void win_draw(win_t* win, const GLfloat* points, size_t len, const GLuint* indexes, size_t ilen) {
    ...
    if (win->VAO_len == 0) {
        glGenVertexArrays(1, &win->VAO[0]);
        win->VAO_len = 1;
    }
    glBindVertexArray(win->VAO[win->VAO_curr]);

    if (win->VBO_len == 0) {
        glGenBuffers(1, &win->VBO[0]);
        win->VBO_len = 1;
    }
    ...
    size_t actual_ilen = 0;
    bool free_indexes = false;

    if (indexes == NULL || ilen == 0) {
        actual_ilen = len / 3; // Assuming triangles
        actual_indexes = (GLuint*)malloc(actual_ilen * sizeof(GLuint));
        if (actual_indexes) {
            for (size_t cc = 0; cc < actual_ilen; ++cc) {
                actual_indexes[cc] = cc;
            }
            free_indexes = true;
        }
    } else {
        actual_indexes = (GLuint*)indexes;
        actual_ilen = ilen;
    }
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, actual_ilen * sizeof(GLuint), actual_indexes, GL_STATIC_DRAW);
    ...
    // Drawing call
    glDrawElements(GL_TRIANGLES, actual_ilen, GL_UNSIGNED_INT, 0);
