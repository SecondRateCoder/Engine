...
float *vec3_largefdot(vec3_t *aptr, float *bptr, const size_t bptrlen, size_t len){
	if(bptrlen != len* 3){return NULL;}
	float *out = _aligned_malloc(len * sizeof(float), 16);
	__m128 _1, _2, _3;
	float temp[4];
	uint8_t cc_;
	for(size_t cc =0; cc < len; cc++){
		//X
		_1 =_mm_load_ps((float[4]){aptr[cc].x* bptr[cc], (cc < len-1? aptr[cc+1].x* bptr[cc+1]: 0), (cc < len-2? aptr[cc+2].x* bptr[cc+2]: 0), (cc < len-3? aptr[cc+3].x* bptr[cc+3]: 0)});
		//Y
		_2 =_mm_load_ps((float[4]){aptr[cc].y* bptr[cc], (cc < len-1? aptr[cc+1].y* bptr[cc+1]: 0), (cc < len-2? aptr[cc+2].y* bptr[cc+2]: 0), (cc < len-3? aptr[cc+3].y* bptr[cc+3]: 0)});
		//Z
		_3 =_mm_load_ps((float[4]){aptr[cc].z* bptr[cc], (cc < len-1? aptr[cc+1].z* bptr[cc+1]: 0), (cc < len-2? aptr[cc+2].z* bptr[cc+2]: 0), (cc < len-3? aptr[cc+3].z* bptr[cc+3]: 0)});
		_mm_store_ps(out+cc, _mm_add_ps(_1, _mm_add_ps(_2, _3)));
	}
	return out;
}
